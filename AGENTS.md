...
1. **Make intent detection actually work.** Two options, both needed:
   ...
   - ~~Add a deterministic **keyword-based router in the CLI** as the primary path~~ **DONE** — `detect_intent` now attempts explicit keyword matching across the user's prompt *before* making an LLM call. This acts as a fast, robust primary path, correctly picking up management intents (like "continue" or "new project") and regular modes based on phrases. The fallback classification still exists: if keywords don't match, THEN it asks the model (with the no-fallback-on-error rule: if the model fails to produce valid JSON, error loudly).
2. **Extend `all_intents()` with management intents** so the router auto-manages state invisibly:
   - ~~`continue`~~ **DONE** → detects phrases like "continue", "resume", "pick up where we left off" to resume the latest workspace/project.
   - ~~`new_project`~~ **DONE** → detects phrases like "new project", "fresh start", "blank slate" to start fresh and auto-create state.
   - ~~`show_work`~~ **DONE** → detects phrases like "show work", "list projects" to list existing stories/projects.
   - These new intents correctly map to target modes (`Mode::Story` or `Mode::Chat`) and remove the need for common users to manually juggle `session`/`workspace` commands.
