...
12. 🟡 **per_handler_grammars** — the JSON envelope of every stage is BNF-constrained (`Schema::to_gbnf`); the prose body of chapter / outline / wiki / synopsis / validation handlers is still free-form and uses pre-fill + strip-think as interim coverage. The domain BNFs (`outline.bnf`, `wiki.bnf`, `chapter_prose.bnf`, `validation_report.bnf`, `synopsis.bnf`) now **exist** in `GBNF/` and are exposed via `roco_grammar::grammar_library` (validated against `roco-bnf-engine`). Remaining: route each prose handler through its grammar so prose is generated **outside** the JSON envelope, with `NO_THINK_PREFILL` (`<think></think>`, `crates/engine/src/backend.rs`) prefilled at the assistant-turn start to suppress `<think>` (experimentally validated via `prompt_probe_eval.rs`); for reasoning stages intentionally prefill `<think>` and strip the trace.
