...
- `1_infer/` — the inference engine (model, quant, state, decoding, structured output)
...
`tool_catelogue` precedes `tool_calling`; `structured_output` lives in `1_infer/`
right after `06_gbnf` because it is a GBNF-usage variant; the core
`tool_execution_loop` is foundational). Files may grow a `User:` section with notes/constraints added
during planning.
