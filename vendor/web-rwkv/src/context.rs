...
use wgpu::{
    ...
    PipelineLayoutDescriptor, PowerPreference, Queue,
...
pub struct Context {
    ...
    /// Cached compiled shaders for faster subsequent loads.
...
pub struct ContextBuilder {
    ...
    pub limits: Limits,
    /// Optional pre-compiled pipeline cache data.
...
impl ContextBuilder {
    pub fn new(adapter: Adapter) -> Self {
        let features = Features::PIPELINE_CACHE;
    ...
    /// Provide previously saved pipeline cache data to speed up shader
    /// compilation on subsequent model loads.
    pub fn with_pipeline_cache(mut self, data: Vec<u8>) -> Self {
    ...
    pub async fn build(self) -> Result<Context, ContextError> {
        ...
        // Create pipeline cache (empty on first run, loaded from disk on subsequent runs)
        // Always created so compiled shaders are captured for subsequent loads.
        let pipeline_cache = Some(unsafe {
...
impl Context {
    /// Get the compiled pipeline cache data for saving to disk.
    /// Pass this to `ContextBuilder::with_pipeline_cache` on next load
    /// to skip shader recompilation.
