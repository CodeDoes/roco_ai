//! Stress-test all 4 prompt styles: run 20 turns through each style
//! and measure output coherence.
//!
//! ```bash
//! RWKV_ADAPTER=NVIDIA cargo run -p roco-core --features local-rwkv \
//!   --example style_stress --release
//! ```
...
#[derive(Clone, Copy)]
enum PromptStyle {
    /// `User: {current}` — bare, relies on RNN state for history
    StateOnly,
    /// `System: ...\nUser: t1\nAssistant: r1\nUser: t2\n...`
    Interleaved,
    /// `User: t1\nAssistant: r1\n...\nSystem: ...\nUser: current`
...
fn repetition(text: &str) -> f64 {
    ...
    for w in words.windows(3) {
        let key = w.join(" ");
        *trip_counts.entry(key).or_insert(0u32) += 1;
    }
    let repeated: u32 = trip_counts.values().filter(|&&c| c > 1).sum();
    repeated as f64 / (words.len() - 2) as f64
}
...
fn build_prompt(
    ...
    match style {
        PromptStyle::StateOnly => current_input.to_string(),
        PromptStyle::Interleaved => {
            ...
            s.push_str(&format!("User: {current_input}"));
            s
        }
        PromptStyle::HistoryFirst => {
...
async fn main() -> anyhow::Result<()> {
    ...
    eprintln!("  {turns_per_style} turns × 4 styles, measuring coherence + repetition.\n");
    ...
    for &style in &PromptStyle::all() {
        ...
        for i in 0..turns_per_style {
            ...
            let req = CompletionRequest {
                ...
                on_token: Some(Box::new(move |t: &str| {
            ...
            let c = coherence(&text);
            let r = repetition(&text);
            scores.push((c, r));
        ...
        let avg_c: f64 = scores.iter().map(|(c, _)| c).sum::<f64>() / scores.len() as f64;
        let avg_r: f64 = scores.iter().map(|(_, r)| r).sum::<f64>() / scores.len() as f64;
        let first_half_c: f64 = scores[..10].iter().map(|(c, _)| c).sum::<f64>() / 10.0;
        let second_half_c: f64 = scores[10..].iter().map(|(c, _)| c).sum::<f64>() / 10.0;
        let drop = first_half_c - second_half_c;

        eprintln!("  ──");
        eprintln!(
            "  Avg coherence={:.3}  repetition={:.3}  trend drop={:.3}",
            avg_c, avg_r, drop
        );
