...
use crate::engine::{
    CompletionRequest, CompletionResponse, ModelBackend, TokenCounter, TokenUsage,
};
...
impl<B: ModelBackend + Send + Sync + ?Sized> Worker<B> {
    ...
    pub async fn execute(&self, task: &Subtask) -> Result<WorkerOutput, AgentError> {
        if let Some(t) = &self.tracer {
            t.record(
                TraceEvent::new(
                    ...
                    format!(
                        "prompt_tokens={} budget_max={}",
                        task.prompt_tokens,
                        self.budget.max_prompt()
                    ),
                )
        ...
        for round in 0..rounds {
            let req = CompletionRequest {
                ...
                grammar: self
                    .tools
                    .as_ref()
                    .map(|tools| tools_to_gbnf_with_think(tools)),
                temperature: 0.2,
                max_tokens: 512,
            ...
            if let Some(t) = &self.tracer {
                ...
                t.record(
                    TraceEvent::new("tool_parse", format!("worker-{}", task.id), &parse_detail)
                        .with_meta(serde_json::json!({
                            "subtask_id": &task.id,
                            "round": round + 1,
        ...
        let resp = final_resp
            .ok_or_else(|| AgentError::Verification("worker produced no model response".into()))?;
        ...
        let parsed = match serde_json::from_str::<Value>(&final_raw) {
            Ok(Value::Object(mut obj)) => {
                obj.insert(
                    ...
                    Self::tool_results_to_json(&tool_results),
                );
                Value::Object(obj)
            }
            Ok(other) => {
...
impl<B: ModelBackend + Send + Sync> Verifier for JudgeVerifier<B> {
    async fn verify(
        ...
        let v: Value = serde_json::from_str(&resp.text)
            .map_err(|e| AgentError::Verification(format!("judge returned invalid JSON: {e}")))?;
        let score =
            (v.get("score").and_then(|x| x.as_f64()).unwrap_or(0.0) as f32 / 10.0).clamp(0.0, 1.0);
