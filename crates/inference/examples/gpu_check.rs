...
fn main() {
    ...
    rt.block_on(async {
        ...
        for (i, adapter) in all.iter().enumerate() {
            ...
            println!("      cooperative_matrix: {}", if has_coop { "YES" } else { "NO" });
            println!("      max_buffer: {:.0} MB", vram_mb);

            println!("\n      Model fit check (rwkv7 2.9B, 5.5GB raw):");
            println!("        no quant (FP16 ~700MB): {}", if vram_mb > 700.0 { "✓ fits" } else { "✗ too small" });
            println!("        Int8 quant (~2.75GB): {}", if vram_mb > 2750.0 { "✓ fits" } else { "✗ too small" });
            println!("        NF4 quant (~1.4GB): {}", if vram_mb > 1400.0 { "✓ fits" } else { "✗ too small" });
        }

        let discrete = all.iter().find(|a| a.get_info().device_type == wgpu::DeviceType::DiscreteGpu);
        ...
        println!("Cooperative matrix: {}", if has_coop { "YES" } else { "NO" });
        if has_coop {
