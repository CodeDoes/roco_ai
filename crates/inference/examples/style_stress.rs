...
#[derive(Clone, Copy)]
enum PromptStyle {
    StateOnly, Interleaved, HistoryFirst, RepeatedSystem,
}
...
impl PromptStyle {
    ...
    fn all() -> [PromptStyle; 4] { [PromptStyle::StateOnly, PromptStyle::Interleaved, PromptStyle::HistoryFirst, PromptStyle::RepeatedSystem] }
}
...
fn coherence(text: &str) -> f64 {
    ...
    let alpha = text.chars().filter(|c| c.is_alphabetic() || c.is_whitespace()).count();
    alpha as f64 / total as f64
}

fn repetition(text: &str) -> f64 {
    let words: Vec<&str> = text.split_whitespace().collect();
    if words.len() < 6 { return 0.0; }
    let mut counts = std::collections::HashMap::new();
    for w in words.windows(3) { *counts.entry(w.join(" ")).or_insert(0u32) += 1; }
...
async fn main() -> anyhow::Result<()> {
    ...
    let inputs: Vec<String> = (0..turns_per_style).map(|i| {
        let qs = ["What is photosynthesis?", "Explain gravity to a child.", "How does a microwave work?",
            "Why is the sky blue?", "What causes earthquakes?", "How do batteries store energy?",
            "Why do leaves change color?", "How does GPS work?", "What is dark matter?",
            "How does the immune system work?", "Explain the internet simply.", "What causes ocean tides?",
            "How does nuclear fusion work?", "Why do we dream?", "How does a computer CPU work?",
            "What is CRISPR gene editing?", "How does WiFi transmit data?", "Why do magnets attract iron?",
            "Explain how vaccines work.", "What is quantum entanglement?"];
    ...
    for &style in &PromptStyle::all() {
        ...
        let mut total_prompt = 0; let mut total_completion = 0;
        let mut scores: Vec<(f64, f64)> = Vec::new();
        for i in 0..turns_per_style {
            eprint!("  {:>2}/{}: ", i + 1, turns_per_style); let _ = std::io::stderr().flush();
            ...
            let resp = backend.complete(CompletionRequest {
                system: String::new(), prompt, output_schema: None, grammar: None,
                temperature: 0.5, max_tokens: 48, estimated_prompt_tokens: 0,
            ...
            scores.push((c, r));
            total_prompt += resp.usage.prompt_tokens; total_completion += resp.usage.completion_tokens;
            eprintln!("{} tok  coherence={:.2}  repetition={:.2}", resp.usage.completion_tokens, c, r);
            if c < 0.9 || r > 0.3 { eprintln!("    ⚠ \"{}\"", text.trim().chars().take(80).collect::<String>()); }
            turns.push(Turn { user: inputs[i].clone(), assistant: text });
        }
        ...
        eprintln!("  Avg coherence={avg_c:.3}  repetition={avg_r:.3}  trend drop={:.3}", first_c - second_c);
