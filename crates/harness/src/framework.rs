...
impl std::fmt::Display for HarnessError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MockNotReady => write!(f, "mock inference not connected"),
            Self::VerificationFailed => write!(f, "verify failed"),
            Self::RollbackError => write!(f, "rollback error"),
        }
    }
}
...
impl MockBackend {
    pub fn generate(&self, prompt: &str) -> String {
        format!("MOCK_INFERENCE_RESULT: {}", prompt.trim())
    }
}
