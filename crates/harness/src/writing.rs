...
impl DomainHarness for WritingAgent {
    ...
    fn init(&mut self, cfg: HarnessConfig) {
        println!("init writing with {:?}", cfg);
    }
    fn run(&self, input: &str, ctx: &Context) -> Result<String, HarnessError> {
        let mock = MockBackend;
        let out = mock.generate(&format!(
            "analyze story: {} session={:?}",
            input, ctx.session_id
        ));
        if out.contains("MOCK") {
            Ok(out)
        } else {
            Err(HarnessError::MockNotReady)
        }
    }
    fn verify(&self, out: &str) -> bool {
        out.contains("MOCK_INFERENCE_RESULT")
    }
    fn rollback(&self, s: &State) -> State {
        State {
...
pub fn detailed_run(input: &str) -> String {
    format!(
        "[MOCK_DETAILED_{}] input_length={} output_generated",
        input,
        input.len()
    )
}
